...
const DIFFS: ToolDiff[] = [
  { add: 13, del: 0, file: "flavors.css" },
  { add: 74, del: 41, file: "ChurnSchedule.tsx" },
  { add: 8, del: 2, file: "menu.ts" },
];
...
const diffLineClass = (tone: ToolDiffLine["tone"]): string => {
  if (tone === "add") {
...
const diffLinePrefix = (tone: ToolDiffLine["tone"]): string => {
  if (tone === "add") {
    return "+";
  }
  if (tone === "del") {
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="rounded-control text-ink-2 hover:bg-hover-2 -mx-1.5 flex w-fit items-center gap-1.5 px-1.5 py-1 text-[12.5px] transition-colors duration-100"
      >
        <svg
          width="12"
          height="12"
      ...
      >
        ...
        <div className="-mx-1 overflow-hidden px-1.5 pb-1">
              ...
              return (
                ...
                >
                  <button
                    ...
                    className="group/row rounded-control hover:bg-hover-2 -mx-[3px] flex h-7 w-[calc(100%+6px)] min-w-0 items-center gap-2 px-[3px] text-left transition-colors duration-100"
                  >
                    <span className="text-ink-3 relative flex size-4 shrink-0 items-center justify-center">
                      <svg
                        width="13"
                        height="13"
                        viewBox="0 0 24 24"
                    ...
                    <span
                      className={`rounded-chip bg-field text-ink-2 shadow-hairline hover:bg-hover-2 inline-flex h-5.5 min-w-0 flex-1 cursor-pointer items-center truncate px-1.5 text-[11.5px] transition-colors duration-100 ${row.mono ? "font-mono" : ""}`}
                    >
                      {row.chip}
                    </span>
                  </button>
                  ...
                  >
                    <div className="min-h-0 overflow-hidden">
                      <div className="border-line mt-0.5 mb-1 ml-2 flex flex-col gap-0.5 border-l py-0.5 pl-3.5">
                        {row.detail.map((line) => (
                          <span
                            key={line.text}
          ...
          {/* file-diff chips */}
          {step >= total && (
            <div className="border-line mt-2.5 flex max-w-full flex-wrap gap-1.5 border-t pt-2.5">
              {diffs.map((d, i) => (
                <span
                  key={d.file}
                  data-diffchip
                ...
                >
                  <button
                    ...
                    className="rounded-chip bg-surface text-ink shadow-btn hover:bg-hover inline-flex h-7 max-w-full items-center gap-2 px-2 font-mono text-[11.5px] transition-colors duration-100"
                    style={{
                      animation: `pop-in 250ms cubic-bezier(0.23,1,0.32,1) ${i * 80}ms both`,
                    }}
                  >
                    <span className="min-w-0 truncate">{d.file}</span>
                    <span className="text-green shrink-0 tabular-nums">
                      +{d.add}
                    </span>
                    {d.del > 0 && (
                      <span className="text-red shrink-0 tabular-nums">
                        −{d.del}
                      </span>
                    )}
                  </button>
              ...
              <button
                type="button"
                className="rounded-chip text-ink-3 hover:text-ink-2 inline-flex h-7 items-center px-1.5 font-mono text-[11.5px] underline decoration-transparent underline-offset-2 transition-colors duration-100 hover:decoration-current"
                style={{
      ...
      {preview &&
        ...
        createPortal(
          <div
            className="bg-surface shadow-overlay fixed z-50 w-72 overflow-hidden rounded-[10px]"
            style={{
          ...
          >
            <div className="border-line flex items-center justify-between border-b px-2.5 py-1.5 font-mono text-[11px]">
            ...
            <div className="py-1 font-mono text-[11px] leading-[1.8]">
              {(diffLines[preview.file] ?? []).map((line) => (
                <div
                  key={`${line.tone}:${line.text}`}
                  className={`flex gap-2 px-2.5 whitespace-pre ${diffLineClass(line.tone)}`}
                >
                  <span className="w-3 shrink-0 select-none">
                    {diffLinePrefix(line.tone)}
                  </span>
