...
`wrangler.jsonc` is the OpenNext Worker config that Workers Builds uploads. It has no D1, KV, or queue ids (those need a real resource id and the default build token cannot invent one). `wrangler.local.jsonc` is only for `db:push`. The Worker already treats these bindings as optional:
