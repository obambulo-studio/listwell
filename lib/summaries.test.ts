    ...
    const filtered = filterToCitedChecks(
      {
        ...
        nextActions: [{ text: "Add photos.", checkIds: ["google-listing-photos"], priority: 1 }],
      },
      checks,
    );

    expect(filtered?.overview).toEqual([{ text: "Title is missing the suburb.", checkIds: ["website-title"] }]);
...
describe("summarizeAuditChecks", () => {
  ...
  it("keeps a cited Workers AI brief and drops invented scores", async () => {
    const ai = {
      run: vi.fn().mockResolvedValue({
        response: JSON.stringify({
          ...
          nextActions: [{ text: "Add the suburb to the title.", checkIds: ["website-title"], priority: 1 }],
        }),
      }),
    };
    ...
    expect(summary.overview.map((claim) => claim.text)).toEqual(["The title is missing the suburb."]);
