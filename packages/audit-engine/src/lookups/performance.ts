...
import { fetchBrowserRenderingHtml } from "../browser";
...
export const parseSyntheticTiming = (
  html: string
): {
  value?: number;
  kind: "lcp" | "load" | "none";
} => {
  const lcp = html.match(/data-listwell-lcp=["']?(?<value>\d+)/iu)?.groups
    ?.value;
  const timing = html.match(/data-listwell-timing=["']?(?<value>\d+)/iu)?.groups
    ?.value;
  const kindRaw = html.match(
    /data-listwell-timing-kind=["']?(?<kind>lcp|load)/iu
  )?.groups?.kind;
  const parsed = htmlLcpSchema.parse({ kind: kindRaw, lcp, timing });
  if (parsed.lcp) {
    return { kind: "lcp", value: Number(parsed.lcp) };
  }
  if (parsed.timing) {
    return { kind: "load", value: Number(parsed.timing) };
  }
  return { kind: "none" };
};

export const performanceFromTiming = (
  ...
  if (value === undefined || kind === "none") {
    return performanceDataSchema.parse({
      kind: "none",
      message:
        "Synthetic browser load did not report LCP or load timing. This is not Chrome UX Report data.",
    });
  }
...
export const fetchPageSpeedPerformance = async (
  url: string,
  googleApiKey: string | undefined,
  fetchImpl: typeof fetch = fetch
): Promise<PerformanceData> => {
...
export const measureSyntheticPerformance = async (
  url: string,
  ...
  if (
    ...
    try {
      const html = await fetchBrowserRenderingHtml(
        url,
  ...
  return performanceDataSchema.parse({
    kind: "none",
    message:
      "Browser Rendering is not configured, so Listwell cannot measure a synthetic LCP. This check does not use CrUX or PageSpeed.",
  });
};
